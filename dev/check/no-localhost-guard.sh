...
LOCALHOST_MATCHES=$(git grep -e localhost --and --not -e '^\s*//' --and --not -e 'CI\:LOCALHOST_OK' -- '*.go' ':(exclude)vendor' ':(exclude)schema' ':(exclude)legacyschema' ':(exclude)cmd/frontend/internal/pkg/langservers/langservers.go' ':(exclude)*_test.go')
